...
// 宏的标准方法。此 DLL 中的所有文件都是用命令行上定义的 SAFETYCARDLIB_EXPORTS
// 符号编译的。在使用此 DLL 的
// 任何其他项目上不应定义此符号。这样，源文件中包含此文件的任何其他项目都会将
// SAFETYCARDLIB_API 函数视为是从 DLL 导入的，而此 DLL 则将用此宏定义的
// 符号视为是被导出的。
...
#define	_GUIZHOU_CA_		0			//贵州CA添加ECC解密，枚举设备和连接设备修改，实现SKF_WaitForDevEvent()
...
*密钥应用类型
*/
#define AT_extern "C"AL_AUTHENTICATE_KEY	0x00000001		//用于设备对终端的认证
...
extern "C"
{
	...
	*  pbDataLen  [OUT] 调用时表示pbData的长度，返回解密数据的大小
	*/
