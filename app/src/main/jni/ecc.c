				   ...
				   )
{
	...
	{	
		...
		__android_log_print(ANDROID_LOG_INFO,"csm_softsm2","i==MAXBNWordLen");
		return 0;
	}
...
//		printf("i==iPlainTextLen\n");
		__android_log_print(ANDROID_LOG_INFO,"csm_softsm2","i==iPlainTextLen2");
		return 0;
	}	
    ...
    for(i=0;i<HASHLEN;i++)
	{
		if(U[i]!=pbC3[i])
			{
				//printf("U[%d]!=pbC3[%d]\n",i,i);
				__android_log_print(ANDROID_LOG_INFO,"csm_softsm2","U[%d]!=pbC3[%d]",i,i);
				free(Xbyte);
				free(Ybyte);	
				free(XMbyte);
				free(XMYbyte);
