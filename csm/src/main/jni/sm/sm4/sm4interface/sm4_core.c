...
mm_u32_t *g_p_table = NULL;

/** 注意 des = des XOR src **/
#define XOR_128BIT(des, src)\
{\
...
/* 线性变换——L变换 */
#define  L_CHANGE(x) ((x) ^ SHL((x),2) ^ SHL((x),10) ^ SHL((x),18)^ SHL((x),24))
...
/* 密钥扩展中用的线性变换 */
#define L_KEY(x)  (x)^SHL((x), 13)^SHL((x), 23)

/* 密钥扩展一轮变换 */
#define SM4_KEY_ONE_ROUND(m0, m1, m2, m3, ck, dec_rk, enc_rk, i, tmp)\
{\
	tmp = m1 ^ m2 ^ m3 ^ (ck)[i];\
	tmp = T_CHANGE(tmp);\
	m0 ^= L_KEY(tmp);\
	dec_rk[SM4_SKEY_NUM - 1 - (i)] =  m0;\
	enc_rk[i] = m0;\
}

/* 将S盒变换和线性变换（L）混合在一起 */
#define  SL_CHANGE_MID_TABLE(x)\
	(	(mtable0[((x)>>24)       ])\
	^	(mtable1[((x)>>16) & 0xff])\
	^	(mtable2[((x)>> 8) & 0xff])\
	^	(mtable3[((x)    ) & 0xff]    )	)
...
/* 使用小表法的一轮变换 */
#define SM4_ONE_ROUND_SMALL_TABLE(x0, x1, x2, x3, sk, tmp)\
	(tmp) = (x1) ^ (x2) ^ (x3) ^ (*(sk));\
	(tmp) = T_CHANGE((tmp));\
	(x0) ^= L_CHANGE((tmp));

/* 使用中表法的一轮变换 */ 
#define SM4_ONE_ROUND_MID_TABLE(x0, x1, x2, x3, sk, tmp){\
	(tmp) = (x1) ^ (x2) ^ (x3) ^ (*(sk));\
	(x0) ^=\
	  *(const mm_u32_t *)( p_des_sp +         (((tmp)>>24)       ))\
	^ *(const mm_u32_t *)( p_des_sp + 0x100 + (((tmp)>>16) & 0xff))\
	^ *(const mm_u32_t *)( p_des_sp + 0x200 + (((tmp)>> 8) & 0xff))\
	^ *(const mm_u32_t *)( p_des_sp + 0x300 + (((tmp)    ) & 0xff));}

 

/* 一轮变换 */
#ifdef SM4_USE_MID_TABLE
	#define SM4_ONE_ROUND		SM4_ONE_ROUND_MID_TABLE 
#else
	#define SM4_ONE_ROUND		SM4_ONE_ROUND_SMALL_TABLE 
#endif
 
#define SM4_FOUR_ROUND(x0, x1, x2, x3, sk, tmp){\
	SM4_ONE_ROUND(x0, x1, x2, x3, sk,   tmp);\
	SM4_ONE_ROUND(x1, x2, x3, x0, sk+1, tmp);\
	SM4_ONE_ROUND(x2, x3, x0, x1, sk+2, tmp);\
	SM4_ONE_ROUND(x3, x0, x1, x2, sk+3, tmp);\
}
 
/* 128比特的数据加载 */
#define MM_LOAD_U128H(u32_0, u32_1, u32_2, u32_3, p_u128)\
{	MM_LOAD_U32H(u32_0, p_u128     );\
	MM_LOAD_U32H(u32_1, p_u128 +  4);\
	MM_LOAD_U32H(u32_2, p_u128 +  8);\
	MM_LOAD_U32H(u32_3, p_u128 + 12);	}

/* 128比特的数据存储 */
#define MM_STORE_U128H(u32_0, u32_1, u32_2, u32_3, p_u128)\
{	MM_STORE_U32H(u32_0, p_u128      );\
	MM_STORE_U32H(u32_1, p_u128 +  4 );\
	MM_STORE_U32H(u32_2, p_u128 +  8 );\
	MM_STORE_U32H(u32_3, p_u128 + 12 ); }
  
static mm_u8_t sbox[256]={
...
#ifdef SM4_USE_MID_TABLE/* 使用中表时 */

static const mm_u32_t sm4_sptrans[4][256]=
{
	{/*SP-0*/
...
#endif/* end #ifndef SM4_USE_MID_TABLE*/  
...
{
	static const mm_u32_t key_cst[4] = 
	{	0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC };

	static const mm_u32_t ck_cst[32]= 
	{  
	...
	register mm_u32_t mk0, mk1, mk2, mk3, i, tmp;   
 
	/* 初始准备 */ /* mk[0,1,2,3] = ch_key[0,1,2,3] ^ key_cst[0,1,2,3] */  
	MM_LOAD_U32H(mk0, ch_key     ); mk0 ^= key_cst[0];
	MM_LOAD_U32H(mk1, ch_key +  4); mk1 ^= key_cst[1]; 
	MM_LOAD_U32H(mk2, ch_key +  8); mk2 ^= key_cst[2];
	MM_LOAD_U32H(mk3, ch_key + 12);	mk3 ^= key_cst[3];  

	for( i = 0; i < SM4_SKEY_NUM; i+=4 )
	{ 
		SM4_KEY_ONE_ROUND(mk0, mk1, mk2, mk3, ck_cst, dec_rk, enc_rk, i,   tmp);
		SM4_KEY_ONE_ROUND(mk1, mk2, mk3, mk0, ck_cst, dec_rk, enc_rk, i+1, tmp);
		SM4_KEY_ONE_ROUND(mk2, mk3, mk0, mk1, ck_cst, dec_rk, enc_rk, i+2, tmp);
		SM4_KEY_ONE_ROUND(mk3, mk0, mk1, mk2, ck_cst, dec_rk, enc_rk, i+3, tmp);
	} 

	mk0 = mk1 = mk2 = mk3 = tmp = 0; 
	return 1; 
}
...
mm_void_t  sm4_enc_dec(mm_u32_t rk[SM4_SKEY_NUM], 
					   mm_u8_t ch_in[SM4_BLOCK_LEN], 
					   mm_u8_t ch_out[SM4_BLOCK_LEN])  
{
	register mm_u32_t  x0, x1, x2, x3, mid; 
	register const mm_u32_t *p_des_sp = (const mm_u32_t *)sm4_sptrans;
	mm_i32_t i = 0;

	/* 数据加载 */
	MM_LOAD_U128H(x0, x1, x2, x3, ch_in); 
 
#ifndef SM4_UNROLL
	for( i = 0; i < 8; i++ )
	{
		SM4_FOUR_ROUND(x0, x1, x2, x3, rk, mid); 
		rk += 4;
	}
#else
	SM4_FOUR_ROUND(x0, x1, x2, x3, rk,    mid); 
	SM4_FOUR_ROUND(x0, x1, x2, x3, rk+ 4, mid); 
	SM4_FOUR_ROUND(x0, x1, x2, x3, rk+ 8, mid); 
	SM4_FOUR_ROUND(x0, x1, x2, x3, rk+12, mid); 
	SM4_FOUR_ROUND(x0, x1, x2, x3, rk+16, mid); 
	SM4_FOUR_ROUND(x0, x1, x2, x3, rk+20, mid); 
	SM4_FOUR_ROUND(x0, x1, x2, x3, rk+24, mid); 
	SM4_FOUR_ROUND(x0, x1, x2, x3, rk+28, mid);  
#endif

	/* 数据存储	- 注意这里的顺序为x3, x2, x1, x0 */
	MM_STORE_U128H(x3, x2, x1, x0, ch_out); 
