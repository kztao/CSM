...
static const char *tag = "csm_jwp11";
//CK_ATTRIBUTE_PTR pTemplate_new = NULL_PTR;
...
static CK_RV (*Pointer_C_GetFunctionList_JW)
(
...
static CK_RV (*Pointer_C_GenerateExchangeKeypair)
(
...
P11func_JW::P11func_JW(){
	libLoadManager = new LibLoadManager(libPath);
	Pointer_C_GetFunctionList_JW = (CK_RV (*)(CK_FUNCTION_LIST_PTR_PTR))libLoadManager->GetFuncPointer("C_GetFunctionList");

	Pointer_C_GetFunctionList_JW(&function_list_ptr);	

	Pointer_C_GenerateExchangeKeypair = (CK_RV (*)(CK_SESSION_HANDLE,CK_MECHANISM_PTR,CK_ATTRIBUTE_PTR,CK_ULONG,CK_ATTRIBUTE_PTR,CK_ULONG,CK_OBJECT_HANDLE_PTR,CK_OBJECT_HANDLE_PTR))libLoadManager->GetFuncPointer("C_GenerateExchangeKeypair");	
...
string P11func_JW::GetDes() {
	return Describe;
}

CK_RV P11func_JW::Adapter_C_Initialize(CK_VOID_PTR   pInitArgs)
{
  	CK_RV ret = 0;
    LOGSERVERI(tag,"%s IN",__FUNCTION__);
    mutex.Lock();
...
CK_RV P11func_JW::Adapter_C_Finalize(CK_VOID_PTR   pReserved)
{
	CK_RV ret = 0;
  	mutex.Lock();
...
CK_RV P11func_JW::Adapter_C_GetInfo(CK_INFO_PTR   pInfo)
{
	CK_RV ret = 0;
  	mutex.Lock();
...
CK_RV P11func_JW::Adapter_C_GetFunctionList(CK_FUNCTION_LIST_PTR_PTR ppFunctionList)
{
	CK_RV ret = 0;
 	mutex.Lock();
...
CK_RV P11func_JW::Adapter_C_GetSlotList(CK_BBOOL       tokenPresent,  /* only slots with tokens */
 ...
 * the system.
 */
CK_RV P11func_JW::Adapter_C_GetSlotInfo(
 ...
 * in the system.
 */
CK_RV P11func_JW::Adapter_C_GetTokenInfo(
 ...
 */
CK_RV P11func_JW::Adapter_C_GetMechanismList(
 ...
 */
CK_RV P11func_JW::Adapter_C_GetMechanismInfo(
  CK_SLOT_ID            slotID,  /* ID of the token's slot */
...
CK_RV P11func_JW::Adapter_C_InitToken(
  CK_SLOT_ID      slotID,    /* ID of the token's slot */
...
CK_RV P11func_JW::Adapter_C_InitPIN(
  CK_SESSION_HANDLE hSession,  /* the session's handle */
...
CK_RV P11func_JW::Adapter_C_SetPIN(
  CK_SESSION_HANDLE hSession,  /* the session's handle */
 ...
 * token.
 */
CK_RV P11func_JW::Adapter_C_OpenSession(
 ...
 * token.
 */
CK_RV P11func_JW::Adapter_C_CloseSession(
  CK_SESSION_HANDLE hSession  /* the session's handle */
)
{
	CK_RV ret = 0;
...
CK_RV P11func_JW::Adapter_C_CloseAllSessions(
...
CK_RV P11func_JW::Adapter_C_GetSessionInfo(
  CK_SESSION_HANDLE   hSession,  /* the session's handle */
 ...
 * in a session.
 */
CK_RV P11func_JW::Adapter_C_GetOperationState(
 ...
 */
CK_RV P11func_JW::Adapter_C_SetOperationState(
  CK_SESSION_HANDLE hSession,            /* session's handle */
...
CK_RV P11func_JW::Adapter_C_Login(
  CK_SESSION_HANDLE hSession,  /* the session's handle */
...
CK_RV P11func_JW::Adapter_C_Logout(
  CK_SESSION_HANDLE hSession  /* the session's handle */
)
{
	CK_RV ret = 0;
...
CK_RV P11func_JW::Adapter_C_CreateObject(
  CK_SESSION_HANDLE hSession,    /* the session's handle */
...
)
{
	...
	{		
//		pTemplate_new = (CK_ATTRIBUTE_PTR)malloc(ulCount*sizeof(CK_ATTRIBUTE));	
 ...
 * copy.
 */
CK_RV P11func_JW::Adapter_C_CopyObject(
  CK_SESSION_HANDLE    hSession,    /* the session's handle */
...
CK_RV P11func_JW::Adapter_C_DestroyObject(
  CK_SESSION_HANDLE hSession,  /* the session's handle */
  CK_OBJECT_HANDLE  hObject    /* the object's handle */
)
{
...
CK_RV P11func_JW::Adapter_C_GetObjectSize(
  CK_SESSION_HANDLE hSession,  /* the session's handle */
  CK_OBJECT_HANDLE  hObject,   /* the object's handle */
 ...
 * attributes.
 */
CK_RV P11func_JW::Adapter_C_GetAttributeValue(
  CK_SESSION_HANDLE hSession,   /* the session's handle */
  CK_OBJECT_HANDLE  hObject,    /* the object's handle */
...
//			CK_ATTRIBUTE_PTR pTemplate_new = (CK_ATTRIBUTE_PTR)malloc(sizeof(CK_ATTRIBUTE) * ulCount);	
			...
			LOGSERVERI(tag, "%s OUT2, rv = 0x%lx",__FUNCTION__,ret);
			return ret;
		}
	}
	...
	LOGSERVERI(tag,"%s OUT with ret = %ld", __FUNCTION__,ret);
	return ret;
 ...
 * attributes.
 */
CK_RV P11func_JW::Adapter_C_SetAttributeValue(
  CK_SESSION_HANDLE hSession,   /* the session's handle */
  CK_OBJECT_HANDLE  hObject,    /* the object's handle */
 ...
 */
CK_RV P11func_JW::Adapter_C_FindObjectsInit(
  CK_SESSION_HANDLE hSession,   /* the session's handle */
	...
	if(pTemplate != NULL_PTR && ulCount != 0)
	{
		...
		if((getclass == CKO_SECRET_KEY && ret != CKK_SESSKEY_EXCHANGE)|| ret == CKK_SM4 || ret == CKK_ZUC)
		{
			...
			if(ret!=0)
			{
				if(ret == ulCount)
				{
					//require some other attributes in find template
					LOGSERVERE(tag,"findobjectinit template Error");
				...
				if(idindex != INVALID_VALUE)
				{
					...
					LOGSERVERI(tag,"tmp_id_len is %lu, idindex is %d", tmp_struct.tmp_id_len,idindex);
					
					memcpy(tmp_struct.tmp_id,(CK_BYTE_PTR)pTemplate[idindex].pValue,tmp_struct.tmp_id_len);
				}
					
				CK_ULONG ulCountNew = ulCount-ret;
				...
				LOGSERVERI(tag,"%s OUT2 with ret 0x%lx, tmp_id_len is %lu", __FUNCTION__,ret,tmp_struct.tmp_id_len);
				return ret;
			}
		}
	}	
 ...
 * handles.
 */
CK_RV P11func_JW::Adapter_C_FindObjects(
 CK_SESSION_HANDLE    hSession,          /* session's handle */
...
)
{
	...
	LOGSERVERI(tag,"%s IN, tmp_id_len is %lu", __FUNCTION__,tmp_struct.tmp_id_len);
	...
	{		
		...
		LOGSERVERI(tag,"%s OUT with ret = 0x%lx, ulObjectCount = %lu", __FUNCTION__,ret,*pulObjectCount);
		return ret;
	}
	
	
	CK_OBJECT_HANDLE_PTR phObject_tmp = (CK_OBJECT_HANDLE_PTR)malloc(ulMaxObjectCount*sizeof(CK_OBJECT_HANDLE));
	...
	mutex.Unlock();
	LOGSERVERI(tag, "find, count is %lu", ulObjectCount_tmp);
	...
	{	
		...
		if(ret != CKR_OK)
		{
			LOGSERVERE(tag, "%s, C_GetAttributeValue ret = 0x%08lx",__FUNCTION__,ret);
			return ret;
		}
				
		nidlen = pTemplate_getnid[0].ulValueLen;
		LOGSERVERI(tag,"index %d, GetAttributeValue, nidlen is %lu",i, nidlen);
		...
		{	
			LOGSERVERI(tag, "get object no sid");
			if(nidlen>0)
			{
		...
		{
			if((hasid != HAS_SID)||(sid != tmp_struct.tmp_sid))
			{
		...
		if(tmp_struct.tmp_id!=NULL_PTR)
		{
			if(tmp_struct.tmp_id_len!=idlen)
			{				
				LOGSERVERI(tag,"id len different, idlen = %lu, tmp_id_len = %lu",idlen,tmp_struct.tmp_id_len);
				continue;
			}
		...
		memcpy(phObject+j,phObject_tmp+i,sizeof(CK_OBJECT_HANDLE));
		j++;
	}
	*pulObjectCount = j;
 ...
 * objects.
 */
CK_RV P11func_JW::Adapter_C_FindObjectsFinal(
  CK_SESSION_HANDLE hSession  /* the session's handle */
)
{	
	LOGSERVERI(tag,"%s IN", __FUNCTION__);
...
CK_RV P11func_JW::Adapter_C_EncryptInit(
  CK_SESSION_HANDLE hSession,    /* the session's handle */
...
)
{
	...
	LOGSERVERI(tag,"%s IN", __FUNCTION__);
/*	LOGSERVERI(tag,"hsession is 0x%x",hSession);
	
	LOGSERVERI(tag,"mech type is 0x%x",pMechanism->mechanism);
	LOGSERVERI(tag,"mech len is %d",pMechanism->ulParameterLen);
	if(pMechanism->pParameter)
	{
		Print_Data((char *)tag,(CK_BYTE_PTR)pMechanism->pParameter,pMechanism->ulParameterLen);
	}
*/	
 	mutex.Lock();	
...
CK_RV P11func_JW::Adapter_C_Encrypt(
  CK_SESSION_HANDLE hSession,            /* session's handle */
 ...
 * operation.
 */
CK_RV P11func_JW::Adapter_C_EncryptUpdate(
  CK_SESSION_HANDLE hSession,           /* session's handle */
 ...
 * operation.
 */
CK_RV P11func_JW::Adapter_C_EncryptFinal(
...
CK_RV P11func_JW::Adapter_C_DecryptInit(
  CK_SESSION_HANDLE hSession,    /* the session's handle */
...
CK_RV P11func_JW::Adapter_C_Decrypt(
  CK_SESSION_HANDLE hSession,           /* session's handle */
 ...
 * operation.
 */
CK_RV P11func_JW::Adapter_C_DecryptUpdate(
  CK_SESSION_HANDLE hSession,            /* session's handle */
 ...
 * operation.
 */
CK_RV P11func_JW::Adapter_C_DecryptFinal(
  CK_SESSION_HANDLE hSession,       /* the session's handle */
...
CK_RV P11func_JW::Adapter_C_DigestInit(
  CK_SESSION_HANDLE hSession,   /* the session's handle */
...
CK_RV P11func_JW::Adapter_C_Digest(
  CK_SESSION_HANDLE hSession,     /* the session's handle */
 ...
 * operation.
 */
CK_RV P11func_JW::Adapter_C_DigestUpdate(
  CK_SESSION_HANDLE hSession,  /* the session's handle */
 ...
 */
CK_RV P11func_JW::Adapter_C_DigestKey(
  CK_SESSION_HANDLE hSession,  /* the session's handle */
 ...
 * operation.
 */
CK_RV P11func_JW::Adapter_C_DigestFinal(
  CK_SESSION_HANDLE hSession,     /* the session's handle */
  CK_BYTE_PTR       pDigest,      /* gets the message digest */
 ...
 * signature.
 */
CK_RV P11func_JW::Adapter_C_SignInit(
  CK_SESSION_HANDLE hSession,    /* the session's handle */
 ...
 */
CK_RV P11func_JW::Adapter_C_Sign(
  CK_SESSION_HANDLE hSession,        /* the session's handle */
 ...
 */
CK_RV P11func_JW::Adapter_C_SignUpdate(
  CK_SESSION_HANDLE hSession,  /* the session's handle */
 ...
 */
CK_RV P11func_JW::Adapter_C_SignFinal(
  CK_SESSION_HANDLE hSession,        /* the session's handle */
  CK_BYTE_PTR       pSignature,      /* gets the signature */
  CK_ULONG_PTR      pulSignatureLen  /* gets signature length */
)
 ...
 */
CK_RV P11func_JW::Adapter_C_SignRecoverInit(
  CK_SESSION_HANDLE hSession,   /* the session's handle */
  CK_MECHANISM_PTR  pMechanism, /* the signature mechanism */
 ...
 */
CK_RV P11func_JW::Adapter_C_SignRecover(
  CK_SESSION_HANDLE hSession,        /* the session's handle */
  CK_BYTE_PTR       pData,           /* the data to sign */
  CK_ULONG          ulDataLen,       /* count of bytes to sign */
  CK_BYTE_PTR       pSignature,      /* gets the signature */
 ...
 */
CK_RV P11func_JW::Adapter_C_VerifyInit(
  CK_SESSION_HANDLE hSession,    /* the session's handle */
 ...
 */
CK_RV P11func_JW::Adapter_C_Verify(
  CK_SESSION_HANDLE hSession,       /* the session's handle */
 ...
 * and plaintext cannot be recovered from the signature.
 */
CK_RV P11func_JW::Adapter_C_VerifyUpdate(
  CK_SESSION_HANDLE hSession,  /* the session's handle */
 ...
 */
CK_RV P11func_JW::Adapter_C_VerifyFinal(
  CK_SESSION_HANDLE hSession,       /* the session's handle */
 ...
 */
CK_RV P11func_JW::Adapter_C_VerifyRecoverInit(
  CK_SESSION_HANDLE hSession,    /* the session's handle */
  CK_MECHANISM_PTR  pMechanism,  /* the verification mechanism */
  CK_OBJECT_HANDLE  hKey         /* verification key */
)
 ...
 * operation, where the data is recovered from the signature.
 */
CK_RV P11func_JW::Adapter_C_VerifyRecover(
  CK_SESSION_HANDLE hSession,        /* the session's handle */
  CK_BYTE_PTR       pSignature,      /* signature to verify */
  CK_ULONG          ulSignatureLen,  /* signature length */
 ...
 */
CK_RV P11func_JW::Adapter_C_DigestEncryptUpdate(
  CK_SESSION_HANDLE hSession,            /* session's handle */
  CK_BYTE_PTR       pPart,               /* the plaintext data */
 ...
 * digesting operation.
 */
CK_RV P11func_JW::Adapter_C_DecryptDigestUpdate(
  CK_SESSION_HANDLE hSession,            /* session's handle */
 ...
 */
CK_RV P11func_JW::Adapter_C_SignEncryptUpdate(
  CK_SESSION_HANDLE hSession,            /* session's handle */
  CK_BYTE_PTR       pPart,               /* the plaintext data */
  CK_ULONG          ulPartLen,           /* plaintext length */
  CK_BYTE_PTR       pEncryptedPart,      /* gets ciphertext */
 ...
 * verify operation.
 */
CK_RV P11func_JW::Adapter_C_DecryptVerifyUpdate(
  CK_SESSION_HANDLE hSession,            /* session's handle */
  CK_BYTE_PTR       pEncryptedPart,      /* ciphertext */
  CK_ULONG          ulEncryptedPartLen,  /* ciphertext length */
  CK_BYTE_PTR       pPart,               /* gets plaintext */
 ...
 * object.
 */
CK_RV P11func_JW::Adapter_C_GenerateKey(
  CK_SESSION_HANDLE    hSession,    /* the session's handle */
 ...
 */
CK_RV P11func_JW::Adapter_C_GenerateKeyPair(
  CK_SESSION_HANDLE    hSession,                    /* session handle */
...
)
{
	...
	for(i=0;i<ulPublicKeyAttributeCount;i++)
	{
		...
		{			
			mutex.Lock();
			ret = Pointer_C_GenerateExchangeKeypair(hSession, pMechanism,pPublicKeyTemplate, ulPublicKeyAttributeCount,
...
CK_RV P11func_JW::Adapter_C_WrapKey(
  CK_SESSION_HANDLE hSession,        /* the session's handle */
 ...
 * key object.
 */
CK_RV P11func_JW::Adapter_C_UnwrapKey(
  CK_SESSION_HANDLE    hSession,          /* session's handle */
...
//		pTemplate_new = (CK_ATTRIBUTE_PTR)malloc(ulAttributeCount*sizeof(CK_ATTRIBUTE));	
	...
	LOGSERVERI(tag, "%s OUT, ret = 0x%08lx",__FUNCTION__,ret);
	return ret;
 ...
 * object.
 */
CK_RV P11func_JW::Adapter_C_DeriveKey(
  CK_SESSION_HANDLE    hSession,          /* session's handle */
  CK_MECHANISM_PTR     pMechanism,        /* key deriv. mech. */
 ...
 */
CK_RV P11func_JW::Adapter_C_SeedRandom(
  CK_SESSION_HANDLE hSession,  /* the session's handle */
...
CK_RV P11func_JW::Adapter_C_GenerateRandom(
  CK_SESSION_HANDLE hSession,    /* the session's handle */
 ...
 * application.
 */
CK_RV P11func_JW::Adapter_C_GetFunctionStatus(
  CK_SESSION_HANDLE hSession  /* the session's handle */
)
{
	CK_RV ret = 0;
 ...
 * running in parallel.
 */
CK_RV P11func_JW::Adapter_C_CancelFunction(
  CK_SESSION_HANDLE hSession  /* the session's handle */
)
{
	CK_RV ret = 0;
 ...
 */
CK_RV P11func_JW::Adapter_C_WaitForSlotEvent(
 ...
 *剩余口令剩余尝试次数
*/
CK_RV P11func_JW::Adapter_C_Extend_GetPinRemainCount
(
  CK_SESSION_HANDLE hSession,
 ...
 *获取密码卡状态
*/
CK_RV P11func_JW::Adapter_C_Extend_GetStatus
(
  CK_SLOT_ID slotID,
 ...
 *注册密码卡状态回调函数
*/
 CK_RV P11func_JW::Adapter_C_Extend_Register_Callback
(
 ...
 *注销密码卡状态回调函数
*/
 CK_RV P11func_JW::Adapter_C_Extend_Unregister_Callback
(
 ...
 *使用监听公钥导出协商密钥
*/
 CK_RV P11func_JW::Adapter_C_Extend_GetExchangeSessionKey
(
  CK_SESSION_HANDLE hSession,
...
)
{
	...
	if(ret != CKR_OK)
	{
		LOGSERVERE(tag,"get sesskeyID fail,hSessionKey is 0x%08lx",hSessionKey);
		return ret;
	}

	LOGSERVERI(tag, "%s, keyid is %d,hSessionKey is 0x%08lx",__FUNCTION__,nSessKeyID,hSessionKey);	
	mutex.Lock();
 ...
 *参数注销
*/
 CK_RV P11func_JW::Adapter_C_Extend_Destroy
(
  CK_SLOT_ID slotID,
 ...
 *重设用户口令
*/
 CK_RV P11func_JW::Adapter_C_Extend_Reset_Pin_With_OTP
(
  CK_SESSION_HANDLE hSession,
 ...
 *重设OTP口令
*/
 CK_RV P11func_JW::Adapter_C_Extend_Reset_OTP
(
  CK_SESSION_HANDLE hSession,
 ...
 *获取剩余OTP解锁次数
*/
 CK_RV P11func_JW::Adapter_C_Extend_Get_OTP_Unlock_Count
(
  CK_SESSION_HANDLE hSession,
...
)
{
	...
	*pulCount = nOTPPinTryTime;
	...
	LOGSERVERI(tag, "%s OUT, ret = 0x%08lx, count = %d,%lu",__FUNCTION__,ret, nOTPPinTryTime,*pulCount);
 ...
 *获取剩余OTP尝试次数
*/
 CK_RV P11func_JW::Adapter_C_Extend_Get_OTP_Remain_Count
(
  CK_SESSION_HANDLE hSession,
  CK_ULONG_PTR pulCount
)
{
	...
	*pulCount = nOTPPinRemainTime;
	...
	LOGSERVERI(tag, "%s OUT, ret = 0x%08lx, count = %d,%lu",__FUNCTION__,ret, nOTPPinRemainTime,*pulCount);
	return ret;
}
 ...
 *协商会话密钥加密初始化
*/
 CK_RV P11func_JW::Adapter_C_Extend_DeriveSessionKey
(
   CK_SESSION_HANDLE hSession,
...
)
{
	...
	LOGSERVERI(tag,"%s IN,hLocalKey 0x%08lx,hRemoteKey 0x%08lx,ulAttributeCount %lu",__FUNCTION__,hLocalKey,hRemoteKey,ulAttributeCount);
 ...
 *协商会话密钥加密初始化
*/
 CK_RV P11func_JW::Adapter_C_Extend_EncryptInit
(
  CK_SESSION_HANDLE hSession,    /* the session's handle */
  CK_MECHANISM_PTR  pMechanism,  /* the encryption mechanism */
  CK_ATTRIBUTE_PTR  pTemplate,
...
)
{
	...
	LOGSERVERI(tag, "%s, hKey is 0x%08lx", __FUNCTION__,hKey[0]);
 ...
 *协商会话密钥解密初始化
*/
 CK_RV P11func_JW::Adapter_C_Extend_DecryptInit
(
  CK_SESSION_HANDLE hSession,    /* the session's handle */
  CK_MECHANISM_PTR  pMechanism,  /* the decryption mechanism */
...
)
{
	...
	if(ret != CKR_OK)
	{	
		LOGSERVERI(tag,"%s, findobjectinit fail",__FUNCTION__);
		return ret;
	}
	...
	if(ret != CKR_OK||ulObjectCount==0)
	{	
		LOGSERVERI(tag,"%s, findobject fail",__FUNCTION__);
 ...
 *协商会话密钥分步加密
*/
 CK_RV P11func_JW::Adapter_C_Extend_EncryptUpdate
(
  CK_SESSION_HANDLE hSession,           /* session's handle */
...
)
{
	...
	CK_MECHANISM ZUCmechanism = {CKM_ZUC_EEA, pIv, ulIvLen};
	
 	mutex.Lock();
	ret = function_list_ptr->C_EncryptInit(hSession,&ZUCmechanism,hKey_enc);
	mutex.Unlock();
	
	LOGSERVERD(tag,"encinit: ret = 0x%lx",ret);
	...
	LOGSERVERD(tag,"%s OUT, ret = 0x%lx, pulEncryptedPartLen = %lu", __FUNCTION__,ret,*pulEncryptedPartLen);
 ...
 *协商会话密钥分步解密
*/
 CK_RV P11func_JW::Adapter_C_Extend_DecryptUpdate
(
  CK_SESSION_HANDLE hSession,            /* session's handle */
...
)
{
	...
	LOGSERVERD(tag,"%s OUT, ret = 0x%lx, pulPartLen = %lu",__FUNCTION__,ret,*pulPartLen);
	return ret;
 ...
 *协商会话密钥分步加密结束
*/
 CK_RV P11func_JW::Adapter_C_Extend_EncryptFinalize
(
  CK_SESSION_HANDLE hSession,                /* session handle */
  CK_BYTE_PTR       pLastEncryptedPart,      /* last c-text */
  CK_ULONG_PTR      pulLastEncryptedPartLen  /* gets last size */
 ...
 *协商会话密钥分步解密结束
*/
 CK_RV P11func_JW::Adapter_C_Extend_DecryptFinalize
(
  CK_SESSION_HANDLE hSession,       /* the session's handle */
  CK_BYTE_PTR       pLastPart,      /* gets plaintext */
  CK_ULONG_PTR      pulLastPartLen  /* p-text size */
 ...
 *SM2点乘
*/
 CK_RV P11func_JW::Adapter_C_Extend_PointMultiply
(
 ...
 *重设TT口令
*/
 CK_RV P11func_JW::Adapter_C_Extend_Reset_TT
(
  CK_SESSION_HANDLE hSession,
  CK_BYTE_PTR pbTTMpk,
  CK_ULONG ulMpkLen,
 ...
 *重设BK口令
*/
 CK_RV P11func_JW::Adapter_C_Extend_Reset_BK
(
  CK_SESSION_HANDLE hSession,
  CK_BYTE_PTR pbBKMpk,
  CK_ULONG ulMpkLen,
...
CK_RV P11func_JW::Adapter_C_Extend_Get_Special_Object_Version
(	
	CK_SESSION_HANDLE hSession,
...
CK_RV P11func_JW::Adapter_C_Extend_DestroyCard
(
	CK_SLOT_ID slotID,
...
CK_RV P11func_JW::Adapter_C_Extend_Get_ExchangePubKey
(
	CK_SESSION_HANDLE hSession,
...
CK_RV P11func_JW::Adapter_C_Extend_GetDevInfo
(
 CK_SLOT_ID slotID,
 const char *userName, 
...
CK_RV P11func_JW::Adapter_C_Extend_DevSign
(
	CK_SLOT_ID slotID,
	CK_BYTE_PTR       pData,           /* the data to sign */
	CK_ULONG          ulDataLen,       /* count of bytes to sign */
...
CK_RV P11func_JW::Adapter_C_Extend_Set_DestroyKey
(
	CK_SESSION_HANDLE hSession,
