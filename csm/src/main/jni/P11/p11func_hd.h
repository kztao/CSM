...
class P11func_HD : public P11AdapterFunction
{
private:
    LibLoadManager *libLoadManager;
    const char *libPath = "libGAPIP11.so";
	string Describe = "hd";
	vector<CK_ATTRIBUTE> dst;
  	Mutex mutex;
	...
	map<CK_SESSION_HANDLE,IV_struct> zucivmap_enc;
	map<CK_SESSION_HANDLE,CK_MECHANISM_TYPE> mechmap_enc;
	map<CK_SESSION_HANDLE,IV_struct> zucivmap_dec;
	map<CK_SESSION_HANDLE,CK_MECHANISM_TYPE> mechmap_dec;
	map<CK_SESSION_HANDLE,int> findtypemap; //0:normal object  1:find bk    2: find ex_keypair  3: find_skey

	
	CK_KEY_TYPE BKKeyType = CKK_SM4;
	CK_BBOOL bktoken = CK_TRUE;
	CK_BBOOL wrapvalue = CK_TRUE;
	CK_BBOOL unwrapvalue = CK_TRUE;
	CK_OBJECT_CLASS keyClass = CKO_SECRET_KEY;
	CK_BYTE nSessKeyBK = CK_SESSKEY_PRESET_ID7;


	CK_ATTRIBUTE BKTemplate[6] = 
	{
		{CKA_CLASS, &keyClass, sizeof(keyClass)},
		{CKA_TOKEN, &bktoken, sizeof(CK_BBOOL)},
		{CKA_KEY_TYPE, &BKKeyType, sizeof(BKKeyType)},
		{CKA_WRAP, &wrapvalue, sizeof(CK_BBOOL)},
		{CKA_UNWRAP, &unwrapvalue, sizeof(CK_BBOOL)},
		{CKA_SESSKEY_ID, &nSessKeyBK, sizeof(nSessKeyBK)}
	};
	

	const CK_ULONG ulcount_ex_keypair = 1;	
	map<CK_SESSION_HANDLE,CK_OBJECT_HANDLE> mapsymkeyhandle;

	
	P11func_HD(const P11func_HD& C);  
	P11func_HD &operator=(const P11func_HD &);
...
CK_RV Adapter_C_Initialize(CK_VOID_PTR   pInitArgs) override;
 ...
 * Cryptoki library.
 */
CK_RV Adapter_C_Finalize(CK_VOID_PTR   pReserved) override;



/* Adapter_C_GetInfo returns general information about Cryptoki. */
CK_RV Adapter_C_GetInfo(CK_INFO_PTR   pInfo) override;



/* Adapter_C_GetFunctionList returns the function list. */
CK_RV Adapter_C_GetFunctionList(CK_FUNCTION_LIST_PTR_PTR ppFunctionList) override;
